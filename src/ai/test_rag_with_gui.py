...
def get_response(user_input):
    response = client.messages.create(
        ...
        system=(
            ...
            "Give brief to medium length responses, unless you get agitated or excited."
...
def get_expert_advice(question, conversation_history):
    # Build a clear, structured transcript
    conversation_transcript = "\n".join(
        f"{'Student' if msg['role'] == 'assistant' else 'Teacher'}: {msg['content']}" 
        for msg in conversation_history
    )
    
    # Retrieve passages using improved criteria
...
Instructions:
Please analyze the teacher's challenge by referring to the above principles. Explain how each principle applies, if at all, and offer specific, actionable strategies that are easy to implement. If the problem seems too advanced for an 8-year-old's perspective, consider providing an alternative approach.
    """
    ...
    response = client.messages.create(
        ...
        max_tokens=1000,
        temperature=0.7,
        system="You are an experienced teacher trainer providing real-time, actionable mentoring. Use the provided teaching principles to inform your advice.",
